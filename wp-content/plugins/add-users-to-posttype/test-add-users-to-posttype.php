				...
				if(!empty($currentsort))
				{
					...
					for($i = 0; $i <$count_total; ++$i)
					{
						...
						if (in_array($post_id, $newUserMeta)) {
							...
							wp_mail( $toUserEmail, 'You\'ve been added to a group', $mailout, $headers );
						}
					...
					foreach($newvalue as $item){
						if (isset($item['user_id']) && $item['user_id'] == $postAuthor->ID){
							$hasAuth = true;
						}
					}
					if($hasAuth != true){
						$array = array();
						$array['user_id'] = $postAuthor->ID;
						$array['user_title'] = '';
						$array['user_info'] = '';
						$array['user_showinfo'] = true;
						$array['user_sortorder'] = count($newvalue) + 1;
