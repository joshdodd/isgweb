...
function wpmem_inc_loginfailed()
{
	$str = '<div align="center" id="wpmem_msg">
		<h2 id="wpmemloginfail">' . __('Login Failed!', 'wp-members') . '</h2>
		<p id="wpmemloginmsg">' . __('You entered an invalid username or password.', 'wp-members') . '</p>
	</div>';
...
function wpmem_login_form_NEW( $page, $arr )
{
	...
	if ( $arr[7] == 'login' ) {
		$form = $form . '
			<div class="remember-reg"><input name="rememberme" type="checkbox" id="rememberme" value="forever" />&nbsp;' . __('Remember me', 'wp-members') . '&nbsp;&nbsp;</div>
			<div class="submit-reg"><input type="submit" name="Submit" value="' . $arr[8] . '" class="buttons" /></div>';
	} else {
	...
	$form = $form . '</div>
			...
			<div class="clear"></div>
			<div align="right" class="pass-reg">';

	if ( ( WPMEM_MSURL != null || $page == 'members' ) && $arr[7] == 'login' ) {

		$link = apply_filters( 'wpmem_forgot_link', wpmem_chk_qstr( WPMEM_MSURL ) . 'a=pwdreset' );
		$form = $form . '<a href="' . $link . '">' . __('Forgot your password?', 'wp-members') . '</a>';

	}

	$form = $form . '</div>
			<div align="right" class="new-reg">';

	if ( ( WPMEM_REGURL != null ) && $arr[7] == 'login' ) {

		$link = apply_filters( 'wpmem_reg_link', WPMEM_REGURL );
		$form = $form . __('Don\'t have an account?', 'wp-members') . '&nbsp;<a href="' . $link . '">' . __('Sign up', 'wp-members') . '</a>';
